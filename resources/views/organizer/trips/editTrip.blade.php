...
<div class="container mx-auto py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Trip</h1>
            <p class="text-gray-600 mt-1">Update details for: {{ $trip->title }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form action="{{ route('trips.update', $trip->id) }}" method="POST" enctype="multipart/form-data" id="tripForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $trip->title) }}" class="form-input @error('title') border-red-500 @enderror" required>
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" rows="6" class="form-input @error('description') border-red-500 @enderror" required>{{ old('description', $trip->description) }}</textarea>
                        @error('description')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="location" name="location" value="{{ old('location', $trip->location) }}" class="form-input @error('location') border-red-500 @enderror" required>
                        @error('location')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group">
                            ...
                            <div class="date-picker-container">
                                ...
                                <input type="hidden" id="start_date" name="start_date" value="{{ old('start_date', $trip->start_date) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <div class="date-picker-container">
                                ...
                                <input type="hidden" id="end_date" name="end_date" value="{{ old('end_date', $trip->end_date) }}" required>
                                <div class="date-picker-icon">
                                    <i class="far fa-calendar-alt"></i>
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="duration" name="duration" value="{{ old('duration', $trip->duration) }}" class="form-input" readonly>
                    </div>
                    
                    <div class="form-group">
                        <label for="price" class="form-label">Price <span class="text-red-500">*</span></label>
                        <input type="number" id="price" name="price" min="0" step="0.01" value="{{ old('price', $trip->price) }}" class="form-input @error('price') border-red-500 @enderror" required>
                        @error('price')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        <label for="max_participants" class="form-label">Max Participants <span class="text-red-500">*</span></label>
                        <input type="number" id="max_participants" name="max_participants" min="1" value="{{ old('max_participants', $trip->max_participants) }}" class="form-input @error('max_participants') border-red-500 @enderror" required>
            ...
            <div class="multi-select-dropdown" id="hotelsDropdown">
                ...
                @foreach ($hotels as $hotel)
                    <div class="multi-select-item" data-value="{{ $hotel->id }}" data-label="{{ $hotel->name }}">
                        <input type="checkbox" id="hotel_{{ $hotel->id }}" name="hotels[]" value="{{ $hotel->id }}" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" 
                        {{ $trip->hotels && $trip->hotels->pluck('id')->contains($hotel->id) ? 'checked' : '' }} hidden>
            ...
            <div class="multi-select-dropdown" id="activitiesDropdown">
                ...
                @foreach ($activities as $activity)
                    <div class="multi-select-item" data-value="{{ $activity->id }}" data-label="{{ $activity->name }}">
                        <input type="checkbox" id="activity_{{ $activity->id }}" name="activities[]" value="{{ $activity->id }}" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" 
                        {{ $trip->activities && $trip->activities->pluck('id')->contains($activity->id) ? 'checked' : '' }} hidden>
        ...
        <div class="transport-gallery" id="transportGallery">
            @foreach ($transports as $transport)
                <div class="transport-item" data-transport-id="{{ $transport->id }}">
                    <input type="radio" id="transport_{{ $transport->id }}" name="transport" value="{{ $transport->id }}" class="hidden" 
                    {{ old('transport', $trip->transport_id) == $transport->id ? 'checked' : '' }}>
        ...
        @enderror
            ...
            <div class="mt-6">
                ...
                <div class="current-image mb-4">
                    <img src="{{ asset('storage/'.$trip->image) }}" alt="{{ $trip->title }}" class="rounded-lg">
                    <div class="current-image-overlay">
                        <button type="button" id="change-image-btn" class="bg-white text-gray-800 hover:bg-gray-100 font-medium py-2 px-4 rounded-lg">
                            Change Image
                        </button>
                    </div>
                </div>
                ...
                <div class="form-group {{ $trip->image ? 'hidden' : '' }}" id="image-upload-container">
                    <label for="image" class="form-label">{{ $trip->image ? 'Change' : 'Featured' }} Image</label>
                ...
                <div class="mt-2 text-sm text-gray-600">
                    <p>{{ $trip->image ? 'Leave empty to keep current image' : 'Upload a featured image for this trip' }}</p>
                </div>
            </div>
            
            <!-- Submit Buttons -->
            <div class="flex justify-end space-x-3 mt-8 pt-5 border-t">
                <a href="{{ route('trips.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 bg-white hover:bg-gray-50">
                    Cancel
                </a>
